...
const SelfDestructMockup = artifacts.require(
  './mockups/SelfDestructMockup.sol'
);
...
module.exports = class TestEnviroment {
  constructor(accounts) {
    ...
    this.testHelpers = Object.assign({}, require('@openzeppelin/test-helpers'));
    this.contracts = {};
  }
  ...
  async setup(accounts) {
    this.contracts.erc20 = await tokenMockup.new(
      'Token name',
      'TKN',
      ether('1000000'),
      this.aliases.admin
    ...
    for (let index = 0; index < 10; index++) {
      await this.contracts.erc20.transfer(accounts[index], ether('1000'), {
        from: user0,
      });
    }
    // mockups
    ...
    this.contracts.treasury = await RCTreasury.new(
      this.contracts.erc20.address
    );
    this.contracts.factory = await RCFactory.new(
      this.contracts.treasury.address,
      this.contracts.realitio.address,
      kleros
    );
    this.contracts.reference = await RCMarket.new();
    this.contracts.orderbook = await RCOrderbook.new(
      this.contracts.treasury.address
    );
    this.contracts.leaderboard = await RCLeaderboard.new(
      this.contracts.treasury.address
    );
    // nft hubs
    ...
    await this.contracts.treasury.setFactoryAddress(
      this.contracts.factory.address
    );
    await this.contracts.treasury.setOrderbookAddress(
      this.contracts.orderbook.address
    );
    await this.contracts.treasury.setLeaderboardAddress(
      this.contracts.leaderboard.address
    );
    await this.contracts.factory.setReferenceContractAddress(
      this.contracts.reference.address
    );
    await this.contracts.factory.setNftHubAddress(
      this.contracts.nftHubL2.address
    );
  ...
  async deployLCClaim(tokenAddress, marketAddress) {
    const claimContract = await LCClaim.new(tokenAddress, marketAddress);
    await this.contracts.erc20.transfer(claimContract.address, ether('10000'), {
      from: user0,
    });
  ...
  async newRental(options) {
    ...
    return await options.market.newRental(
      options.price,
      options.timeLimit,
  ...
  async deposit(amount, user) {
    ...
    await this.contracts.erc20.approve(
      this.contracts.treasury.address,
  ...
  async rentDue(startTx, endTx, price) {
    let startTime = (await web3.eth.getBlock(startTx.receipt.blockNumber))
      .timestamp;
    let endTime = (await web3.eth.getBlock(endTx.receipt.blockNumber))
      .timestamp;
    let rentDue = ether(price)
      .muln(endTime - startTime)
      .divn(86400);
    return rentDue;
  }
  ...
  async newRental(options) {
    ...
    options = this.setDefaults(options, defaults);
    options.price = web3.utils.toWei(options.price.toString(), 'ether');
    let receipt = await options.market.newRental(
      options.price,
      options.timeLimit,
      options.startingPosition,
      options.outcome,
  ...
  async checkOrderbook(expectedResult) {
    ...
    expectedResult.price = web3.utils.toWei(
      expectedResult.price.toString(),
      'ether'
    );
    let exists = await this.contracts.orderbook.bidExists(
      expectedResult.from,
      expectedResult.market.address,
      expectedResult.outcome
    );
    if (!exists) {
      console.log(
        "Bid %s in market %s by user %s doesn't exist",
        expectedResult.outcome,
        expectedResult.market.address,
        expectedResult.from
      );
    ...
    let index = await this.contracts.orderbook.index(
      expectedResult.from,
      expectedResult.market.address,
      expectedResult.outcome
    );
    ...
    assert.equal(
      bid[3],
      expectedResult.outcome,
      'Bid is for incorrect Outcome'
    );
    assert.equal(
      bid[4].toString(),
      expectedResult.price,
      'Bid price is incorrect'
    );
  ...
  async checkOwner(expectedResult) {
    ...
    assert.equal(
      bid[1],
      expectedResult.from,
      "Market doesn't think this is the owner"
    );
    assert.equal(
      bid[3],
      expectedResult.outcome,
  ...
  async printOrderbook(options) {
    ...
    while (!complete) {
      i++;
      index = await this.contracts.orderbook.index(
        user,
        options.market,
        options.outcome
      );
  ...
  async checkCardLists(options) {
    ...
    let record = await this.contracts.orderbook.getBid(
      options.market,
      options.market,
      options.outcome
    );
    ...
    while (record[1] != options.market && success == true) {
      record = await this.contracts.orderbook.getBid(
        options.market,
        record[1],
        options.outcome
      );
      if (
        record[0] != options.market ||
        prevRecord != record[2] ||
        record[3] != options.outcome
      ) {
