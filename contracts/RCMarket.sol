...
contract RCMarket is Initializable, NativeMetaTransaction {
    ...
    event LogReturnToPreviousOwner(
        uint256 indexed tokenId,
    ...
    function _transferCard(
        ...
        require(
            _currentOwner != address(0) && _newOwner != address(0),
    ...
    /// @notice checks whether the Realitio question has resolved, and if yes, gets the winner
    /// @dev can be called by anyone
    function determineWinnerEasy(uint256 _winner)
        external
        ...
        if (cardAffiliateCut > 0) {
            for (uint256 i = 0; i < numberOfTokens; i++) {
                uint256 _cardAffiliatePayment =
                    (collectedPerToken[i].mul(cardAffiliateCut)).div(1000);
    ...
    function rentAllCards(uint256 _maxSumOfPrices) external {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            if (ownerOf(i) != msgSender()) {
                uint256 _newPrice;
                if (price[i] > 0) {
                    _newPrice = (price[i].mul(minimumPriceIncrease.add(100)))
                        .div(100);
                } else {
                    _newPrice = 1 ether;
                }
    ...
    function newRental(
        ...
        uint256 _tokenId
    ) public payable autoUnlock() checkState(States.OPEN) returns (bool) {
        ...
        if (
            _newPrice >=
            (price[_tokenId].mul(minimumPriceIncrease.add(100))).div(100)
        ) {
            collectRentAllCards();
            _validPrice = true;
            ...
            uint256 _minRentalTime =
                uint256(1 days).div(treasury.minRentalDivisor());
            require(
                _timeHeldLimit == 0 ||
                    _timeHeldLimit >=
            ...
            if (_currentOwner == msgSender()) {
                ...
                ownerTracker[_tokenId][currentOwnerIndex[_tokenId]]
                    .price = _newPrice;
            } else {
                ...
                // update internals
                price[_tokenId] = _newPrice;
                currentOwnerIndex[_tokenId] = currentOwnerIndex[_tokenId].add(
                    1
                );
                ownerTracker[_tokenId][currentOwnerIndex[_tokenId]]
                    .price = _newPrice;
    ...
    function sponsor() external payable {
        ...
        collectedPerUser[msgSender()] = collectedPerUser[msgSender()].add(
            msg.value
        );
        // allocate equally to each token, in case card specific affiliates
        for (uint256 i = 0; i < numberOfTokens; i++) {
            collectedPerToken[i] = collectedPerToken[i].add(
    ...
    function _collectRent(uint256 _tokenId) internal {
        ...
        if (ownerOf(_tokenId) != address(this)) {
            uint256 _rentOwed =
                price[_tokenId].mul(now.sub(timeLastCollected[_tokenId])).div(
                    1 days
                );
            address _currentOwner = ownerOf(_tokenId);
            uint256 _cardSpecificDeposit =
                treasury.cardSpecificDeposits(
                    address(this),
                    _currentOwner,
                    _tokenId
            ...
            uint256 _totalDeposit =
                treasury.deposits(_currentOwner).add(_cardSpecificDeposit);
            ...
            if (timeHeldLimit[_tokenId][_currentOwner] == MAX_UINT256) {
                ...
                _rentOwedLimit = price[_tokenId]
                    .mul(
                    timeHeldLimit[_tokenId][_currentOwner].sub(
                        timeHeld[_tokenId][_currentOwner]
                    )
                )
                    .div(1 days);
            }
                ...
                if (_rentOwed >= _totalDeposit || _rentOwed >= _rentOwedLimit) {
                    ...
                    if (_totalDeposit <= _rentOwedLimit) {
                        _timeOfThisCollection = timeLastCollected[_tokenId].add(
                            (
                                (now.sub(timeLastCollected[_tokenId]))
                                    .mul(_totalDeposit)
                                    .div(_rentOwed)
                            )
                        );
            ...
            if (_rentOwed > 0) {
                ...
                timeHeld[_tokenId][_currentOwner] = timeHeld[_tokenId][
                    _currentOwner
                ]
                ...
                collectedPerUser[_currentOwner] = collectedPerUser[
                    _currentOwner
                ]
                    .add(_rentOwed);
                collectedPerToken[_tokenId] = collectedPerToken[_tokenId].add(
                    _rentOwed
                );
    ...
    function _revertToPreviousOwner(uint256 _tokenId) internal {
        ...
        for (uint256 i = 0; i < MAX_ITERATIONS; i++) {
            ...
            if (_index == 0) {
                ...
                _previousOwnersDeposit > 0 &&
                !exitFlag[_previousOwner][_tokenId]
            ) {
                break;
            }
        }
